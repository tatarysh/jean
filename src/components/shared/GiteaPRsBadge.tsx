...
import { useGiteaPRs } from '@/services/gitea'
import { useProjectsStore } from '@/store/projects-store'

const BADGE_STALE_TIME = 5 * 60 * 1000 // 5 minutes — background badge, not active UI

interface GiteaPRsBadgeProps {
  projectId: string
  giteaUrl?: string | null
  className?: string
}

export function GiteaPRsBadge({
  projectId,
  giteaUrl,
  className,
}: GiteaPRsBadgeProps) {
  const { data: prs } = useGiteaPRs(projectId, 'open', {
    enabled: !!giteaUrl,
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{`${totalCount} open Gitea PR${totalCount > 1 ? 's' : ''}`}</TooltipContent>
    </Tooltip>
  )
}
